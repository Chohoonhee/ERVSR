...
class Network(nn.Module):
    def __init__(self, config, spynet_path=None):
       ...
       # Bidirectionaal Propagation
        ...
        # self.ref_down_resblocks = ConvResBlock_ref(3, config.num_feat, config.num_blocks)
    ...
    def forward(self, lrs, refs, is_first_frame, is_log=False, is_train=False):
        ...
        # if refs.size()[-2] == lrs.size()[-2]:
        #     refs = F.interpolate(refs, size=(2*h, 2*w), mode='bicubic')
        # else: 
        #     pass
        ...
        for i in range(0, t):
            ...
            sr_rlt = self.decoder_attention([sr_rlt, ref_fea])[0]
